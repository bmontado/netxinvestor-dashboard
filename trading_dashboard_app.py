...
if uploaded_file:
    ...
    if 'RGL' in xls.sheet_names:
        ...
        if header_row_index is not None:
            ...
            df_data.columns = df_data.columns.str.strip()  # limpiar espacios

            # Verificar que las columnas necesarias existan antes de continuar
            required_cols = ['Symbol', 'Gain/Loss']
            if all(col in df_data.columns for col in required_cols):
                df_data = df_data.dropna(subset=required_cols, how='any')
                ...
                st.error("No se encontraron las columnas necesarias ('Symbol' y 'Gain/Loss') en el archivo.")
        else:
